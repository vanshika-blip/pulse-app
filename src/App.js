...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .refresh-btn { background: transparent; border: 1px solid var(--border); color: var(--muted2);
                 ...
                 font-family: 'Space Mono', monospace; margin-left: auto; display: flex; align-items: center;
  ...
  .modal-section { font-size: 9px; color: var(--green); text-transform: uppercase; letter-spacing: 2px; font-weight: 700; margin: 18px 0 10px; }
  ...
  .inp-label { font-size: 10px; color: var(--muted2); margin-bottom: 5px; display: block; text-transform: uppercase; letter-spacing: 1px; font-weight: 700; }
...
export default function App() {
  ...
  const saveHist = (h) => {
    setHistory(h);
    localStorage.setItem("h_hist", JSON.stringify(h));
  };
  ...
  // ── Generate AI comments via backend ─────────────────────────────────────────
      ...
      {feedError && (
        <div className="error-banner">
          ...
          <button onClick={fetchPosts} style={{
            ...
            color: "#EF4444", borderRadius: 4, padding: "4px 12px", fontSize: 11,
      ...
      <div className="filterbar">
        {[["all","All"],["twitter","X / Twitter"],["reddit","Reddit"],["linkedin","LinkedIn"]].map(([v, l]) => (
          <button key={v} className={`chip${filter === v ? " on" : ""}`} onClick={() => setFilter(v)}>
            {v !== "all" && PLATFORMS[v] && (() => { const I = PLATFORMS[v].Icon; return <I />; })()}
            {l}
          </button>
        ))}
          ...
          {feedPosts.map((post, i) => {
            ...
            return (
              <div key={post.id} className="widget" style={{ animationDelay: `${i * 0.04}s` }}>
                ...
                {post.title && (
                  <div className="w-title" style={{ padding: "0 14px 4px" }}>{post.title}</div>
                )}
            ...
            return (
              <div key={i} className="hist-card">
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{
                    background: `${P.color}18`, color: P.color, display: "flex", alignItems: "center",
    ...
    <div className="page">
      ...
      <div className="settings-card">
        ...
        <div style={{ fontSize: 11, color: "var(--muted2)" }}>
          To change, edit <code>BACKEND_URL</code> at the top of App.js
        </div>
      </div>

      <div className="settings-card">
        <div className="cc-title">OpenAI API Key</div>
        <div className="cc-sub">For AI comment generation — stored locally in your browser</div>
        <label className="inp-label">API Key</label>
        <input className="inp" type="password" placeholder="sk-proj-..." value={apiKey}
      ...
      <div className="settings-card">
        ...
        <div style={{ fontSize: 12, color: "var(--muted2)", lineHeight: 1.8 }}>
          Open <code>server.js</code> and add feeds to the <code>RSS_FEEDS</code> array, then restart:
          <pre style={{
            background: "var(--surf2)", border: "1px solid var(--border)", borderRadius: 6,
        ...
        <div className="modal">
          ...
          {aiComments.length > 0 && (
            <>
              ...
              {aiComments.map((c, i) => (
                <div key={i} className={`c-opt${chosen === c ? " sel" : ""}`}
                  onClick={() => { setChosen(c); setCustom(""); }}>
          ...
          <nav className="sidenav">
            ...
            {TABS.map(t => (
              <button key={t.id} className={`sidenav-btn${tab === t.id ? " active" : ""}`} onClick={() => setTab(t.id)}>
                ...
                {t.id === "feed" && pendingN > 0 && (
                  <span className="badge" style={{ marginLeft: "auto" }}>{pendingN}</span>
                )}
              </button>
            ))}
            ...
            {Object.entries(PLATFORMS).map(([k, P]) => {
              ...
              return (
                <button key={k}
                  className={`sidenav-btn${filter === k && tab === "feed" ? " active" : ""}`}
                  ...
                  {count > 0 && (
                    <span className="badge" style={{ marginLeft: "auto", background: P.color }}>{count}</span>
                  )}
                </button>
              );
            })}
            ...
            <div className="sidenav-stats">
              <div style={{ fontSize: 11, color: "var(--muted2)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10, fontWeight: 700 }}>
                Stats
              </div>
