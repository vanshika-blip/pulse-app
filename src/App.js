...
const RedditIcon = () => (<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 0 1-2.498.056l-2.597-.547-.8 3.747c1.824.07 3.48.632 4.674 1.488.308-.309.73-.491 1.207-.491.968 0 1.754.786 1.754 1.754 0 .716-.435 1.333-1.01 1.614a3.111 3.111 0 0 1 .042.52c0 2.694-3.13 4.87-7.004 4.87-3.874 0-7.004-2.176-7.004-4.87 0-.183.015-.366.043-.534A1.748 1.748 0 0 1 4.028 12c0-.968.786-1.754 1.754-1.754.463 0 .898.196 1.207.49 1.207-.883 2.878-1.43 4.744-1.487l.885-4.182a.342.342 0 0 1 .14-.197.35.35 0 0 1 .238-.042l2.906.617a1.214 1.214 0 0 1 1.108-.701zM9.25 12C8.561 12 8 12.562 8 13.25c0 .687.561 1.248 1.25 1.248.687 0 1.248-.561 1.248-1.249 0-.688-.561-1.249-1.249-1.249zm5.5 0c-.687 0-1.248.561-1.248 1.25 0 .687.561 1.248 1.249 1.248.688 0 1.249-.561 1.249-1.249 0-.687-.562-1.249-1.25-1.249zm-5.466 3.99a.327.327 0 0 0-.231.094.33.33 0 0 0 0 .463c.842.842 2.484.913 2.961.913.477 0 2.105-.056 2.961-.913a.361.361 0 0 0 .029-.463.33.33 0 0 0-.464 0c-.547.533-1.684.73-2.512.73-.828 0-1.979-.196-2.512-.73a.326.326 0 0 0-.232-.095z"/></svg>);
...
function exportCSV(rows) {
  ...
  const lines = [headers.join(","), ...rows.map(r => [
    r.commentedBy||"Anonymous", r.platform, r.author, r.url,
    `"${(r.content||"").replace(/"/g,'""')}"`,
    `"${(r.comment||"").replace(/"/g,'""')}"`,
    r.commentedAt,
  ].join(","))];
...
// ── CSS defined OUTSIDE component so it never changes ─────────────────────────
const css = `
  @import url('https://fonts.googleapis.com/css2?family=Space+Mono:ital,wght@0,400;0,700;1,400&display=swap');
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  :root{--bg:#000;--surf:#0A0A0A;--surf2:#111;--border:rgba(0,255,135,0.15);--green:#00FF87;--green2:#00CC6A;--text:#E0FFE8;--muted2:#3A7A50;--r:10px}
  ...
  .hdr-title h1{font-size:18px;letter-spacing:3px;text-transform:uppercase;color:var(--green);text-shadow:0 0 20px rgba(0,255,135,0.5)}
  ...
  .sidenav{width:220px;background:var(--surf);border-right:1px solid var(--border);padding:24px 12px;display:flex;flex-direction:column;gap:4px;position:sticky;top:57px;height:calc(100vh - 57px);overflow:auto}
  .sidenav-btn{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:6px;background:none;border:none;color:var(--muted2);cursor:pointer;font-size:12px;font-family:'Space Mono',monospace;width:100%;text-align:left;transition:all .2s}
  ...
  .chip{background:var(--surf);border:1px solid var(--border);color:var(--muted2);border-radius:4px;padding:5px 14px;font-size:11px;font-weight:700;cursor:pointer;transition:all .2s;display:flex;align-items:center;gap:6px;font-family:'Space Mono',monospace}
  .chip:hover{border-color:var(--green);color:var(--text)}
  .chip.on{background:var(--green);color:#000;border-color:var(--green)}
  .refresh-btn{background:transparent;border:1px solid var(--border);color:var(--muted2);border-radius:4px;padding:5px 14px;font-size:11px;font-weight:700;cursor:pointer;font-family:'Space Mono',monospace;margin-left:auto;display:flex;align-items:center;gap:6px;transition:all .2s}
  ...
  .widget{break-inside:avoid;margin-bottom:14px;background:var(--surf);border:1px solid var(--border);border-radius:var(--r);overflow:hidden;transition:transform .15s,box-shadow .15s,border-color .15s;animation:fadeUp .4s ease both}
  ...
  .btn-skip{background:transparent;border:1px solid var(--border);color:var(--muted2);border-radius:4px;padding:5px 11px;font-size:10px;cursor:pointer;font-family:'Space Mono',monospace;font-weight:700;transition:all .15s}
  .btn-skip:hover{border-color:var(--muted2);color:var(--text)}
  .btn-remove{background:transparent;border:1px solid rgba(239,68,68,.25);color:#EF4444;border-radius:4px;padding:5px 11px;font-size:10px;cursor:pointer;font-family:'Space Mono',monospace;font-weight:700;transition:all .15s}
  .btn-remove:hover{background:rgba(239,68,68,.1);border-color:#EF4444}
  .btn-comment{background:var(--green);color:#000;border:none;border-radius:4px;padding:5px 13px;font-size:10px;font-weight:700;cursor:pointer;font-family:'Space Mono',monospace;transition:all .15s}
  .btn-comment:hover{background:var(--green2)}
  .bottom-nav{display:none;position:fixed;bottom:0;left:0;right:0;background:var(--surf);border-top:1px solid var(--border);padding:8px 0 14px;z-index:200}
  ...
  .overlay{position:fixed;inset:0;background:rgba(0,0,0,.88);z-index:500;display:flex;align-items:flex-end;justify-content:center;backdrop-filter:blur(4px);animation:fadeIn .2s ease}
  @media(min-width:600px){.overlay{align-items:center}}
  @keyframes fadeIn{from{opacity:0}to{opacity:1}}
  .modal{background:var(--surf);border:1px solid var(--border);border-radius:10px 10px 0 0;width:100%;max-width:560px;max-height:90vh;overflow-y:auto;padding:24px 20px;animation:slideUp .25s ease;position:relative}
  ...
  .gen-btn{width:100%;background:#050505;border:1px solid var(--green);color:var(--green);border-radius:6px;padding:13px;font-size:13px;font-weight:700;cursor:pointer;font-family:'Space Mono',monospace;display:flex;align-items:center;justify-content:center;gap:8px;transition:all .2s;margin-top:16px}
  ...
  .c-area{width:100%;background:var(--surf2);border:1px solid var(--border);border-radius:6px;padding:12px 14px;color:var(--text);font-size:12px;line-height:1.6;resize:none;font-family:'Space Mono',monospace;margin-bottom:14px;min-height:90px}
  .c-area:focus{outline:none;border-color:var(--green)}
  .post-btn{width:100%;background:var(--green);color:#000;border:none;border-radius:6px;padding:15px;font-size:14px;font-weight:700;cursor:pointer;font-family:'Space Mono',monospace;transition:all .2s}
  .post-btn:hover{background:var(--green2)}
  .close-btn{position:absolute;top:20px;right:20px;background:var(--surf2);border:1px solid var(--border);color:var(--muted2);border-radius:4px;width:30px;height:30px;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center}
  ...
  .export-btn{background:transparent;border:1px solid var(--green);color:var(--green);border-radius:4px;padding:8px 18px;font-size:11px;font-weight:700;cursor:pointer;font-family:'Space Mono',monospace;display:inline-flex;align-items:center;gap:6px;margin-bottom:16px}
  ...
  .inp{width:100%;background:var(--surf2);border:1px solid var(--border);border-radius:6px;padding:10px 14px;color:var(--text);font-size:12px;font-family:'Space Mono',monospace;margin-bottom:12px}
  .inp:focus{outline:none;border-color:var(--green)}
  .save-btn{background:var(--green);color:#000;border:none;border-radius:6px;padding:11px 20px;font-size:12px;font-weight:700;cursor:pointer;font-family:'Space Mono',monospace;width:100%}
  ...
  .error-banner{margin:16px 20px;background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.3);border-radius:8px;padding:14px 16px;font-size:12px;color:#EF4444;display:flex;align-items:center;gap:10px;flex-wrap:wrap}
  ...
  .toast{position:fixed;top:70px;left:50%;transform:translateX(-50%);background:var(--surf);border:1px solid var(--border);border-radius:6px;padding:10px 20px;font-size:12px;font-weight:700;z-index:1000;white-space:nowrap;animation:slideDown .25s ease;font-family:'Space Mono',monospace}
...
// ── Sub-components defined OUTSIDE App to prevent re-mount on every keystroke ──

const FeedView = memo(({ posts, loadingFeed, feedError, filter, setFilter, fetchPosts, lastRefresh, onSkip, onRemove, onComment, username, setTab }) => {
  ...
  return (
    <>
      ...
      {!username && (
        <div style={{background:"rgba(0,255,135,0.06)",border:"1px solid var(--border)",borderRadius:8,padding:"12px 16px",margin:"12px 20px",fontSize:11,color:"var(--muted2)",display:"flex",alignItems:"center",gap:10}}>
          ⚠ Set your name in <strong onClick={() => setTab("settings")} style={{cursor:"pointer",color:"var(--green)",marginLeft:4}}>Settings</strong> so your comments are attributed to you
        </div>
      )}
      {feedError && (
        <div className="error-banner">
          <span>⚠ Backend not reachable: {feedError}</span>
          <button onClick={fetchPosts} style={{marginLeft:"auto",background:"transparent",border:"1px solid #EF4444",color:"#EF4444",borderRadius:4,padding:"4px 12px",fontSize:11,cursor:"pointer",fontFamily:"Space Mono,monospace",fontWeight:700}}>Retry</button>
        </div>
      )}
      ...
      {feedPosts.length===0 && !loadingFeed ? (
        <div className="empty">
          ...
          <p>{feedError ? "Check your backend on Render" : "No pending posts."}</p>
        </div>
      ) : (
        <div className="grid">
            ...
            return (
              <div key={post.id} className="widget" style={{animationDelay:`${i*0.04}s`}}>
                ...
                <div className="w-foot">
                  <a href={post.url} target="_blank" rel="noreferrer" style={{fontSize:10,color:"var(--muted2)",textDecoration:"none"}} onClick={e=>e.stopPropagation()}>↗ View</a>
                  <div className="w-actions">
                    <button className="btn-skip" onClick={() => onSkip(post.id)}>Skip</button>
                    <button className="btn-remove" onClick={() => onRemove(post.id)}>✕ Remove</button>
                    <button className="btn-comment" onClick={() => onComment(post)}>Comment ✦</button>
                  </div>
                </div>
              </div>
            );
...
const HistoryView = memo(({ history }) => (
  <div className="page">
    ...
    {history.length > 0 && <button className="export-btn" onClick={() => exportCSV(history)}>↓ Export CSV</button>}
    {history.length === 0 ? (
      <div className="empty">
        <div style={{fontSize:52,marginBottom:16}}>◷</div>
          ...
          return (
            <div key={i} className="hist-card">
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                <span style={{background:`${P.color}18`,color:P.color,display:"flex",alignItems:"center",gap:5,padding:"3px 9px",borderRadius:20,fontSize:11,fontWeight:700}}>
                  <P.Icon/>{P.name}
                </span>
                <span className="pill pill-ok">✓ Saved</span>
              </div>
              <div style={{fontSize:11,color:"var(--green)",fontWeight:700,marginBottom:4}}>👤 {item.commentedBy||"Anonymous"}</div>
              <div style={{fontSize:12,color:"var(--muted2)",marginBottom:6}}>on post by {item.authorName} · {timeAgo(item.commentedAt||item.savedAt)} ago</div>
              ...
              {item.url && <a href={item.url} target="_blank" rel="noreferrer" style={{fontSize:10,color:"var(--muted2)",display:"block",marginTop:6,textDecoration:"none"}}>↗ View original post</a>}
...
const SettingsView = memo(({ feedError, posts, fetchPosts, fetchHistory, usernameInput, setUsernameInput, saveUsername }) => (
  <div className="page">
    ...
    <div className="settings-card">
      ...
      <div style={{background:"var(--surf2)",border:"1px solid var(--border)",borderRadius:6,padding:"12px 14px",fontSize:12,display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
...
const CommentModal = memo(({ commentModal, selected, setCommentModal, generate, loadingAI, aiComments, chosen, setChosen, custom, setCustom, postComment, username }) => {
    ...
    <div className="overlay" onClick={e => { if (e.target.className==="overlay") setCommentModal(false); }}>
      <div className="modal">
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
          <span style={{background:`${P.color}18`,color:P.color,display:"flex",alignItems:"center",gap:5,padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:700}}>
            <P.Icon/> {P.name}
          </span>
        ...
        {!username && (
          <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.3)",borderRadius:6,padding:"10px 14px",fontSize:11,color:"#F97316",marginTop:12}}>
...
export default function App() {
  ...
  const postComment = useCallback(async () => {
    ...
    // Refresh history from DB so everyone's comments show
    await fetchHistory();
    setPosts(p => p.map(x => x.id===selected.id ? {...x, status:"commented"} : x));
    showToast("Comment saved ✓");
    setCommentModal(false);
    setSelected(null);
  }, [custom, chosen, selected, username, showToast, fetchHistory]);
  ...
  return (
    <>
      ...
      <CommentModal
        commentModal={commentModal} selected={selected} setCommentModal={setCommentModal}
        generate={generate} loadingAI={loadingAI} aiComments={aiComments}
        chosen={chosen} setChosen={setChosen} custom={custom} setCustom={setCustom}
        postComment={postComment} username={username}
      />
      <div className="app">
        <div className="hdr">
          <PulseLogo size={36}/>
          <div className="hdr-title"><h1>PULSE</h1><p>Stay in Conversation</p></div>
        ...
        <div className="layout">
          ...
          <main className="main">
            {tab==="feed" && <FeedView posts={posts} loadingFeed={loadingFeed} feedError={feedError}
              filter={filter} setFilter={setFilter} fetchPosts={fetchPosts} lastRefresh={lastRefresh}
              onSkip={skipPost} onRemove={removePost} onComment={openComment} username={username} setTab={setTab}/>}
            {tab==="history" && <HistoryView history={history}/>}
            {tab==="settings" && <SettingsView feedError={feedError} posts={posts} fetchPosts={fetchPosts}
              fetchHistory={fetchHistory} usernameInput={usernameInput} setUsernameInput={setUsernameInput} saveUsername={saveUsername}/>}
          </main>
        </div>
